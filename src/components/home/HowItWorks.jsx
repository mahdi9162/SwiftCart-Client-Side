...
const HowItWorks = () => {
  return (
    <section className="relative w-full px-3 md:px-0 py-20 bg-slate-950 text-white border-t border-white/5">
      <div className="relative z-10 mx-auto max-w-7xl">
        ...
        <div className="max-w-3xl mb-8 md:mb-16">
          ...
          <h2 className="mt-4 text-2xl md:text-6xl font-bold tracking-tight">
            How it works <span className="text-white/30 font-light italic">— seamlessly.</span>
          </h2>
        </div>

        {/* Organized Steps Grid with Borders */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-0 border border-white/10 rounded-2xl overflow-hidden bg-white/1">
          {steps.map((s, idx) => (
            <div
              key={s.title}
              className={`group p-10 flex flex-col justify-between transition-all duration-500  bg-white/3 
            ...
            >
              <div className="space-y-8">
                ...
                <div className="space-y-3">
                  ...
                  <p className="text-sm text-white/40 leading-relaxed max-w-xs">{s.desc}</p>
                </div>
              </div>

              {/* Decorative detail to make it feel less "empty" */}
              <div className="mt-12 flex items-center gap-2 opacity-100 md:opacity-0 group-hover:opacity-100 transition-opacity duration-500">
        ...
        {/* Minimal Footer CTA */}
        <div className="mt-8 md:mt-16 pt-10 border-t border-white/5 flex flex-col sm:flex-row items-center gap-6">
          ...
          <div className="flex gap-4">
            <Link href="/add-item" className="text-sm font-bold text-white hover:text-emerald-400 transition-colors">
              Add Item →
            </Link>
            <Link href="/items/lists" className="text-sm font-bold text-white hover:text-emerald-400 transition-colors">
              Browse Catalog →
            </Link>
          </div>
        </div>
